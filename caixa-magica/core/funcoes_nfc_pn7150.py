...
_CMD_POLL = 'sudo {nfc_demo_app_path} poll'
_CMD_WRITE = '{nfc_demo_app_path} write --type=Text -l en -r "{new_text}"'
#_NFC_DEMO_APP_NAME = 'nfcDemoApp'
#_NFC_DEMO_APP_DEFAULT_LOCATION = '/home/pi/nfc_pn7150/linux_libnfc-nci' #'/usr/sbin'
_NFC_DEMO_APP_NAME = 'nfcCaixaMagica'
_NFC_DEMO_APP_DEFAULT_LOCATION = '/home/pi/caixa-magica/nfc/' #'/usr/sbin'
...
class PN7150(object):
    ...
    def _read_thread(self):
        ...
        while self._read_running:
            try:
                print("Entrando NFC: " + str(datetime.utcnow()))
                line = stdout.readline()
                if _OUTPUT_TEXT in line:
                    print("Comecei: " + str(datetime.utcnow()))
                    first = line.find("'")
                    last = line.rfind("'")
                    text = line[first + 1:last]
                    if self.when_tag_read:
                        self.when_tag_read(text)
                        print("Acabei: " + str(datetime.utcnow()))
